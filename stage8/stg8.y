...
var:    var ',' ID      {if(Glookup($3->NAME)!=NULL)
						{printf("multiple decl\n");exit(0);}
						Ginstall($3->NAME,type,sizeof(int));}
        | ID       {if(Glookup($1->NAME)!=NULL)
						{printf("multiple decl\n");exit(0);}
						Ginstall($1->NAME,type,sizeof(int));}
        | var ',' ID '[' NUM ']' 
        				{if($5->TYPE!=type_int)
    					{printf("array wrong type\n");exit(0);}
    					if(Glookup($3->NAME)!=NULL)
						{printf("multiple decl\n");exit(0);}
						Ginstall($3->NAME,type,sizeof(int)*$4->VALUE);
    					}
        | ID '[' NUM ']' 
        				{if($3->TYPE!=type_int)
    					{printf("array wrong type\n");exit(0);}
						...
						{printf("multiple decl\n");exit(0);}
						Ginstall($1->NAME,type,sizeof(int)*$4->VALUE);
    					}
...
main :START slist END {codegen($2);fprintf(fp, "MOV SP, %d\n",address );
...
stmt :	ID '[' expr ']' ASGN expr ';' 
	 ...
	 | READ '(' ID '[' expr ']' ')' ';' 
         ...
         /*| WRITE '(' ID '[' expr ']' ')' ';'   	
     						...
     						$$= TreeCreate(type_void,WRITEARR,0,NULL,NULL,$3,$5,NULL);}*/
	 | WRITE '('expr ')' ';' { 	
	 							if($3->TYPE==type_bool)
	 							{
